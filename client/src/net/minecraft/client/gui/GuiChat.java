...
public class GuiChat extends GuiScreen {
	...
	public void initGui() {
		...
		String text = inputField == null ? defaultInputFieldText : inputField.getText();
	...
	protected void keyTyped(char typedChar, int keyCode) throws IOException {
		...
		if (keyCode == 15) this.autocompletePlayerNames();
		else this.playerNamesFound = false;

		if (keyCode == 1) this.mc.displayGuiScreen(null);
		else if (keyCode != 28 && keyCode != 156) {
			if (keyCode == 200) this.getSentHistory(-1);
			else if (keyCode == 208) this.getSentHistory(1);
			else if (keyCode == 201) this.mc.ingameGUI.getChatGUI().scroll(this.mc.ingameGUI.getChatGUI().getLineCount() - 1);
			else if (keyCode == 209) this.mc.ingameGUI.getChatGUI().scroll(-this.mc.ingameGUI.getChatGUI().getLineCount() + 1);
			else this.inputField.textboxKeyTyped(typedChar, keyCode);
		} else {
			String s = this.inputField.getText().trim();
			if (s.length() > 0) this.sendChatMessage(s);
