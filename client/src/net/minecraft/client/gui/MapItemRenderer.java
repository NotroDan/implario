...
public class MapItemRenderer {
	...
	class Instance {
		...
		private void render(boolean noOverlayRendering) {
			...
			G.enableBlend();
			G.tryBlendFuncSeparate(1, 771, 0, 1);
			G.disableAlpha();
			...
			for (Vec4b vec4b : this.mapData.mapDecorations.values()) {
				if (!noOverlayRendering || vec4b.func_176110_a() == 1) {
					G.pushMatrix();
					G.translate((float) i + (float) vec4b.func_176112_b() / 2.0F + 64.0F, (float) j + (float) vec4b.func_176113_c() / 2.0F + 64.0F, -0.02F);
					G.rotate((float) (vec4b.func_176111_d() * 360) / 16.0F, 0.0F, 0.0F, 1.0F);
					G.scale(4.0F, 4.0F, 3.0F);
					G.translate(-0.125F, 0.125F, 0.0F);
