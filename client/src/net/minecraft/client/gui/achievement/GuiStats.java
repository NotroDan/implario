...
public class GuiStats extends GuiScreen implements IProgressMeter {
	...
	public GuiStats(GuiScreen parentScreen, StatFileWriter statFile) {
	 ...
	 */
	public void initGui() {
		this.screenTitle = Lang.format("gui.stats");
	...
	public void handleMouseInput() throws IOException {
		...
		if (this.displaySlot != null) this.displaySlot.handleMouseInput();
	}
	...
	public void createButtons() {
		this.buttonList.add(new GuiButton(0, this.width / 2 + 4, this.height - 28, 150, 20, Lang.format("gui.done")));
		this.buttonList.add(new GuiButton(1, this.width / 2 - 160, this.height - 52, 80, 20, Lang.format("stat.generalButton")));
		GuiButton guibutton;
		this.buttonList.add(guibutton = new GuiButton(2, this.width / 2 - 80, this.height - 52, 80, 20, Lang.format("stat.blocksButton")));
		GuiButton guibutton1;
		this.buttonList.add(guibutton1 = new GuiButton(3, this.width / 2, this.height - 52, 80, 20, Lang.format("stat.itemsButton")));
		GuiButton guibutton2;
		this.buttonList.add(guibutton2 = new GuiButton(4, this.width / 2 + 80, this.height - 52, 80, 20, Lang.format("stat.mobsButton")));

		if (this.blockStats.getSize() == 0) guibutton.enabled = false;

		if (this.itemStats.getSize() == 0) guibutton1.enabled = false;

		if (this.mobStats.getSize() == 0) guibutton2.enabled = false;
	}
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		if (button.enabled) if (button.id == 0) this.mc.displayGuiScreen(this.parentScreen);
		else if (button.id == 1) this.displaySlot = this.generalStats;
		else if (button.id == 3) this.displaySlot = this.itemStats;
		else if (button.id == 2) this.displaySlot = this.blockStats;
		else if (button.id == 4) this.displaySlot = this.mobStats;
		else this.displaySlot.actionPerformed(button);
	}
	...
	private void drawSprite(int x1, int y1, int x2, int y2) {
		...
		worldrenderer.pos(x1, y1 + 18, this.zLevel).tex((float) x2 * k, (float) (y2 + 18) * k).endVertex();
		worldrenderer.pos(x1 + 18, y1 + 18, this.zLevel).tex((float) (x2 + 18) * k, (float) (y2 + 18) * k).endVertex();
		worldrenderer.pos(x1 + 18, y1 + 0, this.zLevel).tex((float) (x2 + 18) * k, (float) y2 * k).endVertex();
		worldrenderer.pos(x1, y1, this.zLevel).tex((float) (x2 + 0) * k, (float) (y2 + 0) * k).endVertex();
		tessellator.draw();
	}
	...
	abstract class Stats extends GuiSlot {
		...
		protected void drawListHeader(int x, int y, Tessellator t) {
			if (!Mouse.isButtonDown(0)) this.activeTab = -1;

			if (this.activeTab == 0) GuiStats.this.drawSprite(x + 115 - 18, y + 1, 0, 0);
			else GuiStats.this.drawSprite(x + 115 - 18, y + 1, 0, 18);

			if (this.activeTab == 1) GuiStats.this.drawSprite(x + 165 - 18, y + 1, 0, 0);
			else GuiStats.this.drawSprite(x + 165 - 18, y + 1, 0, 18);

			if (this.activeTab == 2) GuiStats.this.drawSprite(x + 215 - 18, y + 1, 0, 0);
			else GuiStats.this.drawSprite(x + 215 - 18, y + 1, 0, 18);

			if (this.field_148217_o != -1) {
				int i = 79;
				int j = 18;

				if (this.field_148217_o == 1) i = 129;
				else if (this.field_148217_o == 2) i = 179;

				if (this.field_148215_p == 1) j = 36;

				GuiStats.this.drawSprite(x + i, y + 1, j, 0);
			}
		}
		...
		protected void func_148132_a(int p_148132_1_, int p_148132_2_) {
			...
			if (p_148132_1_ >= 79 && p_148132_1_ < 115) this.activeTab = 0;
			else if (p_148132_1_ >= 129 && p_148132_1_ < 165) this.activeTab = 1;
			else if (p_148132_1_ >= 179 && p_148132_1_ < 215) this.activeTab = 2;

			if (this.activeTab >= 0) {
				this.func_148212_h(this.activeTab);
		...
		protected void func_148142_b(int p_148142_1_, int p_148142_2_) {
			if (p_148142_2_ >= this.top && p_148142_2_ <= this.bottom) {
				...
				if (i >= 0) {
					if (p_148142_1_ < j + 40 || p_148142_1_ > j + 40 + 20) return;
					...
					if (p_148142_1_ >= j + 115 - 18 && p_148142_1_ <= j + 115) s = this.func_148210_b(0);
					else if (p_148142_1_ >= j + 165 - 18 && p_148142_1_ <= j + 165) s = this.func_148210_b(1);
					else {
						if (p_148142_1_ < j + 215 - 18 || p_148142_1_ > j + 215) return;

						s = this.func_148210_b(2);
					}

					s = ("" + Lang.format(s)).trim();
		...
		protected void func_148213_a(StatCrafting p_148213_1_, int p_148213_2_, int p_148213_3_) {
			if (p_148213_1_ != null) {
				...
				String s1 = ("" + Lang.format(s + ".name")).trim();
		...
		protected void func_148212_h(int p_148212_1_) {
			...
			} else if (this.field_148215_p == -1) this.field_148215_p = 1;
			else {
	...
	class StatsBlock extends GuiStats.Stats {
		...
		public StatsBlock(Minecraft mcIn) {
			...
			for (StatCrafting statcrafting : StatList.objectMineStats) {
				...
				if (GuiStats.this.statFileWriter.readStat(statcrafting) > 0) flag = true;
				else if (StatList.objectUseStats[i] != null && GuiStats.this.statFileWriter.readStat(StatList.objectUseStats[i]) > 0) flag = true;
				else if (StatList.objectCraftStats[i] != null && GuiStats.this.statFileWriter.readStat(StatList.objectCraftStats[i]) > 0) flag = true;

				if (flag) this.statsHolder.add(statcrafting);
			}
		...
		protected void drawListHeader(int x, int y, Tessellator t) {
			super.drawListHeader(x, y, t);

			if (this.activeTab == 0) GuiStats.this.drawSprite(x + 115 - 18 + 1, y + 1 + 1, 18, 18);
			else GuiStats.this.drawSprite(x + 115 - 18, y + 1, 18, 18);

			if (this.activeTab == 1) GuiStats.this.drawSprite(x + 165 - 18 + 1, y + 1 + 1, 36, 18);
			else GuiStats.this.drawSprite(x + 165 - 18, y + 1, 36, 18);

			if (this.activeTab == 2) GuiStats.this.drawSprite(x + 215 - 18 + 1, y + 1 + 1, 54, 18);
			else GuiStats.this.drawSprite(x + 215 - 18, y + 1, 54, 18);
		}
	...
	class StatsItem extends GuiStats.Stats {
		...
		public StatsItem(Minecraft mcIn) {
			...
			for (StatCrafting statcrafting : StatList.itemStats) {
				...
				if (GuiStats.this.statFileWriter.readStat(statcrafting) > 0) flag = true;
				else if (StatList.objectBreakStats[i] != null && GuiStats.this.statFileWriter.readStat(StatList.objectBreakStats[i]) > 0) flag = true;
				else if (StatList.objectCraftStats[i] != null && GuiStats.this.statFileWriter.readStat(StatList.objectCraftStats[i]) > 0) flag = true;
		...
		protected void drawListHeader(int x, int y, Tessellator t) {
			...
			if (this.activeTab == 0) GuiStats.this.drawSprite(x + 115 - 18 + 1, y + 1 + 1, 72, 18);
			else GuiStats.this.drawSprite(x + 115 - 18, y + 1, 72, 18);

			if (this.activeTab == 1) GuiStats.this.drawSprite(x + 165 - 18 + 1, y + 1 + 1, 18, 18);
			else GuiStats.this.drawSprite(x + 165 - 18, y + 1, 18, 18);

			if (this.activeTab == 2) GuiStats.this.drawSprite(x + 215 - 18 + 1, y + 1 + 1, 36, 18);
			else GuiStats.this.drawSprite(x + 215 - 18, y + 1, 36, 18);
		}
	...
	class StatsMobsList extends GuiSlot {
		...
		protected void drawSlot(int entryID, int p_180791_2_, int p_180791_3_, int p_180791_4_, int mouseXIn, int mouseYIn) {
			...
			if (i == 0) s1 = Lang.format("stat.entityKills.none", s);
