...
public class GuiEnchantment extends GuiContainer {
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		G.viewport((scaledresolution.getScaledWidth() - 320) / 2 * scaledresolution.getScaleFactor(), (scaledresolution.getScaledHeight() - 240) / 2 * scaledresolution.getScaleFactor(),
		...
		for (int j = 0; j < 3; ++j) {
			...
			if (this.isPointInRegion(60, 14 + 19 * j, 108, 17, mouseX, mouseY) && k > 0 && l >= 0) {
				...
				if (l >= 0 && Enchantment.getEnchantmentById(l & 255) != null) {
					...
					list.add(EnumChatFormatting.WHITE.toString() + EnumChatFormatting.ITALIC.toString() + Lang.format("container.enchant.clue", new Object[] {s}));
				}
				...
				if (!flag) {
					...
					if (this.mc.thePlayer.experienceLevel < k) {
						...
						if (i1 == 1) {
							s1 = Lang.format("container.enchant.lapis.one", new Object[0]);
						} else {
							s1 = Lang.format("container.enchant.lapis.many", new Object[] {i1});
						}
						...
						if (i1 == 1) {
							s1 = Lang.format("container.enchant.level.one", new Object[0]);
						} else {
							s1 = Lang.format("container.enchant.level.many", new Object[] {i1});
						}
