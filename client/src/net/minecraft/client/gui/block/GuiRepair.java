...
public class GuiRepair extends GuiContainer implements ICrafting {
	...
	public GuiRepair(InventoryPlayer inventoryIn, World worldIn) {
		super(new ContainerRepair(inventoryIn, worldIn, Minecraft.get().thePlayer));
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		G.disableLighting();
		G.disableBlend();
		this.fontRendererObj.drawString(Lang.format("container.repair"), 60, 6, 4210752);

		if (this.anvil.maximumCost > 0) {
			int i = 8453920;
			boolean flag = true;
			String s = Lang.format("container.repair.cost", this.anvil.maximumCost);

			if (this.anvil.maximumCost >= 40 && !this.mc.thePlayer.capabilities.isCreativeMode) {
				s = Lang.format("container.repair.expensive");
				i = 16736352;
