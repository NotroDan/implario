...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
	...
	public void handleJoinGame(S01PacketJoinGame packetIn) {
		PacketThreadUtil.syncPacket(packetIn, this, this.gameController);
		...
		this.clientWorldController = new WorldClient(this, new WorldSettings(0L, packetIn.getGameType(), false, packetIn.isHardcoreMode(), packetIn.getWorldType()), packetIn.getDimension(),
				packetIn.getDifficulty(), gameController.getProfiler());
	...
	public void handleSpawnObject(S0EPacketSpawnObject packetIn) {
		...
		switch (packetIn.getType()) {
			case 10:
				entity = EntityMinecart.func_180458_a(this.clientWorldController, d0, d1, d2, EntityMinecart.EnumMinecartType.byNetworkID(packetIn.getXi()));
				break;
			case 90:
			...
			case 61:
				...
				break;
			case 71:
				entity = new EntityItemFrame(this.clientWorldController, new BlockPos(MathHelper.floor_double(d0), MathHelper.floor_double(d1), MathHelper.floor_double(d2)),
						EnumFacing.getHorizontal(packetIn.getXi()));
				packetIn.setXi(0);
				break;
			case 65:
			...
			case 76:
				...
				break;
			case 63:
				entity = new EntityLargeFireball(this.clientWorldController, d0, d1, d2, (double) packetIn.getSpeedX() / 8000.0D, (double) packetIn.getSpeedY() / 8000.0D,
						(double) packetIn.getSpeedZ() / 8000.0D);
				packetIn.setXi(0);
				break;
			case 64:
				entity = new EntitySmallFireball(this.clientWorldController, d0, d1, d2, (double) packetIn.getSpeedX() / 8000.0D, (double) packetIn.getSpeedY() / 8000.0D,
						(double) packetIn.getSpeedZ() / 8000.0D);
				packetIn.setXi(0);
				break;
			case 66:
				entity = new EntityWitherSkull(this.clientWorldController, d0, d1, d2, (double) packetIn.getSpeedX() / 8000.0D, (double) packetIn.getSpeedY() / 8000.0D,
						(double) packetIn.getSpeedZ() / 8000.0D);
				packetIn.setXi(0);
				break;
			case 62:
				...
				break;
			case 73:
				entity = new EntityPotion(this.clientWorldController, d0, d1, d2, packetIn.getXi());
				packetIn.setXi(0);
				break;
			case 75:
			...
			case 2:
				...
				break;
			case 70:
				entity = new EntityFallingBlock(this.clientWorldController, d0, d1, d2, Block.getStateById(packetIn.getXi() & 65535));
				packetIn.setXi(0);
				break;
		}

		if (entity != null) addEntityToWorld(packetIn, entity);
	}
	...
	public void addEntityToWorld(S0EPacketSpawnObject packetIn, Entity entity) {
		...
		if (packetIn.getXi() > 0) {
			if (packetIn.getType() == 60) {
				Entity entity2 = this.clientWorldController.getEntityByID(packetIn.getXi());
	...
	public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
		...
		this.netManager.sendPacket(
				new C03PacketPlayer.C06PacketPlayerPosLook(entityplayer.posX, entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, entityplayer.rotationYaw, entityplayer.rotationPitch,
						false));
	...
	public void handleChunkData(S21PacketChunkData packet) {
		PacketThreadUtil.syncPacket(packet, this, this.gameController);
		...
		chunk.fillChunk(packet.getData(), packet.getExtractedSize(), packet.func_149274_i());
	...
	public void handleRespawn(S07PacketRespawn packetIn) {
		...
		if (packetIn.getDimensionID() != this.gameController.thePlayer.dimension) {
			...
			this.clientWorldController = new WorldClient(this, new WorldSettings(0L, packetIn.getGameType(), false, this.gameController.theWorld.getWorldInfo().isHardcoreModeEnabled(),
					packetIn.getWorldType()), packetIn.getDimensionID(), packetIn.getDifficulty(), gameController.getProfiler());
	...
	public void handleResourcePack(S48PacketResourcePackSend packetIn) {
		...
		if (s.startsWith("level://")) {
			...
			//TODO: где то ты накосячил с жестким переносом всего
			//Изначально было this.gameController.addScheduledTask
			this.gameController.integratedServer.addScheduledTask(() -> this.gameController.displayGuiScreen(new GuiYesNo((result, id) -> {
				...
				for (S20PacketEntityProperties.Snapshot s20packetentityproperties$snapshot : packetIn.func_149441_d()) {
					...
					if (iattributeinstance == null)
						iattributeinstance = baseattributemap.registerAttribute(
								new RangedAttribute(null, s20packetentityproperties$snapshot.func_151409_a(), 0.0D, 2.2250738585072014E-308D, Double.MAX_VALUE));
