...
public class WorldRenderer {
	...
	private void func_181670_b(int p_181670_1_) {
		if (Config.isShaders()) p_181670_1_ *= 2;

		if (p_181670_1_ > this.rawIntBuffer.remaining()) {
			int i = this.byteBuffer.capacity();
			int j = i % 2097152;
			int k = j + (((this.rawIntBuffer.position() + p_181670_1_) * 4 - j) / 2097152 + 1) * 2097152;
			Log.MAIN.warn("Needed to grow BufferBuilder buffer: Old size " + i + " bytes, new size " + k + " bytes.");
	...
	public void setVertexState(WorldRenderer.State state) {
		...
		if (state.stateQuadSprites != null) {
			...
			if (this.quadSprites == null || this.quadSprites.length < this.getBufferQuadSize()) this.quadSprites = new TextureAtlasSprite[this.getBufferQuadSize()];
	...
	public void putColorRGBA(int index, int red, int p_178972_3_, int p_178972_4_, int p_178972_5_) {
		if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN) this.rawIntBuffer.put(index, p_178972_5_ << 24 | p_178972_4_ << 16 | p_178972_3_ << 8 | red);
	...
	private void func_181667_k() {
		...
		if (this.field_181677_f.getUsage() == VertexFormatElement.EnumUsage.PADDING) this.func_181667_k();
	}
	...
	public void drawMultiTexture() {
		if (this.quadSprites == null) return;
