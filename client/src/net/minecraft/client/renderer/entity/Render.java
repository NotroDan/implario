...
public abstract class Render<T extends Entity> {
	...
	public boolean shouldRender(T livingEntity, ICamera camera, double camX, double camY, double camZ) {
		...
		if (axisalignedbb.func_181656_b() || axisalignedbb.getAverageEdgeLength() == 0.0D)
			axisalignedbb = new AxisAlignedBB(livingEntity.posX - 2.0D, livingEntity.posY - 2.0D, livingEntity.posZ - 2.0D, livingEntity.posX + 2.0D, livingEntity.posY + 2.0D,
	...
	protected void renderName(T entity, double x, double y, double z) {
		if (this.canRenderName(entity)) this.renderLivingLabel(entity, entity.getDisplayName().getFormattedText(), x, y, z, 64);
	}
	...
	private void renderEntityOnFire(Entity entity, double x, double y, double z, float partialTicks) {
		...
		G.translate(0.0F, 0.0F, -0.3F + (float) (int) f3 * 0.02F);
	...
	private void renderShadow(Entity entityIn, double x, double y, double z, float shadowAlpha, float partialTicks) {
		if (!Config.isShaders() || !Shaders.shouldSkipDefaultShadow) {
			...
			if (entityIn instanceof EntityLivingBase) {
				if (((EntityLivingBase) entityIn).isChild()) f *= 0.5F;
			}
			...
			for (BlockPos blockpos : BlockPos.getAllInBoxMutable(new BlockPos(i, k, i1), new BlockPos(j, l, j1))) {
				...
				if (block.getRenderType() != -1 && world.getLightFromNeighbors(blockpos) > 3) this.func_180549_a(block, x, y, z, blockpos, shadowAlpha, f, d2, d3, d4);
			}
	...
	private void func_180549_a(Block blockIn, double p_180549_2_, double p_180549_4_, double p_180549_6_, BlockPos pos, float p_180549_9_, float p_180549_10_, double p_180549_11_, double p_180549_13_,
							   double p_180549_15_) {
	...
	public void doRenderShadowAndFire(Entity entityIn, double x, double y, double z, float yaw, float partialTicks) {
		if (Settings.ENTITY_SHADOWS.b() && this.shadowSize > 0.0F && !entityIn.isInvisible() && this.renderManager.isRenderShadow()) {
	 ...
	 */
	public MCFontRenderer getFontRendererFromRenderManager() {
	...
	protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance) {
		...
		if (d0 <= (double) (maxDistance * maxDistance)) {
			...
			G.pushMatrix();
			G.translate((float) x + 0.0F, (float) y + entityIn.height + 0.5F, (float) z);
