...
public class Timer {
	...
	public void updateTimer() {
		...
		long dsync = time - this.lastSyncSysClock;
		long realTime = System.nanoTime() / 1000000L;
		double d0 = (double) realTime / 1000.0D;

		if (dsync <= 1000L && dsync >= 0L) {
			this.field_74285_i += dsync;

			if (this.field_74285_i > 1000L) {
				long l = realTime - this.lastSyncHRClock;
				double d1 = (double) this.field_74285_i / (double) l;
				this.timeSyncAdjustment += (d1 - this.timeSyncAdjustment) * 0.20000000298023224D;
				this.lastSyncHRClock = realTime;
