...
public class CustomColors {
	...
	private static Pair<CustomColormap[], Integer> parseLightmapsRgb() {
		...
		Integer[] ainteger = set.toArray(Utils.INTEGER);
		...
		return new ImmutablePair(acustomcolormap, j1);
	}
	...
	private static CustomColormap[] readCustomColormaps(Properties p_readCustomColormaps_0_, String p_readCustomColormaps_1_) {
		...
		String[] astring = (String[]) map.keySet().toArray(Utils.STRING);
		...
		for (String s6 : astring) {
			...
			if (customcolormap == null)
				...
				if (amatchblock != null && amatchblock.length > 0) {
					for (MatchBlock matchblock : amatchblock) customcolormap.addMatchBlock(matchblock);
		...
		if (list.size() <= 0)
			return null;
		return (CustomColormap[]) list.toArray(Utils.CUSTOMCOLORMAP);
	}
	...
	private static CustomColormap[][] readBlockColormaps(String[] p_readBlockColormaps_0_, CustomColormap[] p_readBlockColormaps_1_, int p_readBlockColormaps_2_, int p_readBlockColormaps_3_) {
		...
		if (p_readBlockColormaps_1_ != null)
			for (CustomColormap customcolormap1 : p_readBlockColormaps_1_) addToBlockList(customcolormap1, list);
	...
	private static CustomColormap[][] blockListToArray(List l) {
		CustomColormap[][] acustomcolormap = new CustomColormap[l.size()][];

		for (int i = 0; i < l.size(); ++i) {
			List list = (List) l.get(i);

			if (list != null) {
				CustomColormap[] acustomcolormap1 = (CustomColormap[]) list.toArray(Utils.CUSTOMCOLORMAP);
	...
	private static CustomColormap getCustomColors(String p_getCustomColors_0_, String[] p_getCustomColors_1_, int p_getCustomColors_2_, int p_getCustomColors_3_) {
		for (String aP_getCustomColors_1_ : p_getCustomColors_1_) {
			String s = aP_getCustomColors_1_;
	...
	public static int getColorMultiplier(BakedQuad p_getColorMultiplier_0_, Block p_getColorMultiplier_1_, IBlockAccess p_getColorMultiplier_2_, BlockPos p_getColorMultiplier_3_,
										 RenderEnv p_getColorMultiplier_4_) {
		...
		return Config.isSmoothBiomes() && !customcolors$icolorizer.isColorConstant() ? getSmoothColorMultiplier(p_getColorMultiplier_2_, p_getColorMultiplier_3_, customcolors$icolorizer,
	...
	private static int getSmoothColorMultiplier(IBlockAccess p_getSmoothColorMultiplier_0_, BlockPos p_getSmoothColorMultiplier_1_, CustomColors.IColorizer p_getSmoothColorMultiplier_2_,
												BlockPosM p_getSmoothColorMultiplier_3_) {
		int i = 0;
		int j = 0;
		int k = 0;
		...
		for (int k1 = l - 1; k1 <= l + 1; ++k1)
			for (int l1 = j1 - 1; l1 <= j1 + 1; ++l1) {
				p_getSmoothColorMultiplier_3_.setXyz(k1, i1, l1);
				int i2 = p_getSmoothColorMultiplier_2_.getColor(p_getSmoothColorMultiplier_0_, p_getSmoothColorMultiplier_3_);
				i += i2 >> 16 & 255;
				j += i2 >> 8 & 255;
				k += i2 & 255;
			}
	...
	public static int getFluidColor(IBlockAccess p_getFluidColor_0_, IBlockState p_getFluidColor_1_, BlockPos p_getFluidColor_2_, RenderEnv p_getFluidColor_3_) {
		...
		return customcolors$icolorizer == null ? block.colorMultiplier(p_getFluidColor_0_,
				p_getFluidColor_2_) : Config.isSmoothBiomes() && !customcolors$icolorizer.isColorConstant() ? getSmoothColorMultiplier(p_getFluidColor_0_, p_getFluidColor_2_, customcolors$icolorizer,
				p_getFluidColor_3_.getColorizerBlockPosM()) : customcolors$icolorizer.getColor(p_getFluidColor_0_, p_getFluidColor_2_);
	}
	...
	private static int getRedstoneLevel(IBlockState state, int i) {
		Block block = state.getBlock();

		if (!(block instanceof BlockRedstoneWire)) return i;
		Integer object = state.getValue(BlockRedstoneWire.POWER);
		if (!(object instanceof Integer)) return i;
		return object;
	}
	...
	public static Vec3d getUnderwaterColor(IBlockAccess p_getUnderwaterColor_0_, double p_getUnderwaterColor_1_, double p_getUnderwaterColor_3_, double p_getUnderwaterColor_5_) {
		...
		float f1 = (float) k / 255.0F;
		float f2 = (float) l / 255.0F;
		return underwaterColorFader.getColor((double) f, (double) f1, (double) f2);
	}

	private static int getStemColorMultiplier(Block p_getStemColorMultiplier_0_, IBlockAccess p_getStemColorMultiplier_1_, BlockPos p_getStemColorMultiplier_2_,
	...
	public static boolean updateLightmap(World p_updateLightmap_0_, float p_updateLightmap_1_, int[] p_updateLightmap_2_, boolean p_updateLightmap_3_) {
		...
		if (j >= 0 && j < lightMapsColorsRgb.length) {
			...
			float f3 = Config.limitTo1(Settings.GAMMA.f());
	...
	private static float[][] readDyeColors(Properties p_readDyeColors_0_, String p_readDyeColors_1_, String p_readDyeColors_2_, String p_readDyeColors_3_) {
		...
		for (EnumDyeColor enumdyecolor : aenumdyecolor) map.put(enumdyecolor.getName(), enumdyecolor);
	...
	private static int getMapColorIndex(String p_getMapColorIndex_0_) {
		return p_getMapColorIndex_0_ == null ? -1 : p_getMapColorIndex_0_.equals("air") ? MapColor.airColor.colorIndex : p_getMapColorIndex_0_.equals(
				"grass") ? MapColor.grassColor.colorIndex : p_getMapColorIndex_0_.equals("sand") ? MapColor.sandColor.colorIndex : p_getMapColorIndex_0_.equals(
				"cloth") ? MapColor.clothColor.colorIndex : p_getMapColorIndex_0_.equals("tnt") ? MapColor.tntColor.colorIndex : p_getMapColorIndex_0_.equals(
				"ice") ? MapColor.iceColor.colorIndex : p_getMapColorIndex_0_.equals("iron") ? MapColor.ironColor.colorIndex : p_getMapColorIndex_0_.equals(
				"foliage") ? MapColor.foliageColor.colorIndex : p_getMapColorIndex_0_.equals("clay") ? MapColor.clayColor.colorIndex : p_getMapColorIndex_0_.equals(
				"dirt") ? MapColor.dirtColor.colorIndex : p_getMapColorIndex_0_.equals("stone") ? MapColor.stoneColor.colorIndex : p_getMapColorIndex_0_.equals(
				"water") ? MapColor.waterColor.colorIndex : p_getMapColorIndex_0_.equals("wood") ? MapColor.woodColor.colorIndex : p_getMapColorIndex_0_.equals(
				"quartz") ? MapColor.quartzColor.colorIndex : p_getMapColorIndex_0_.equals("gold") ? MapColor.goldColor.colorIndex : p_getMapColorIndex_0_.equals(
				"diamond") ? MapColor.diamondColor.colorIndex : p_getMapColorIndex_0_.equals("lapis") ? MapColor.lapisColor.colorIndex : p_getMapColorIndex_0_.equals(
				"emerald") ? MapColor.emeraldColor.colorIndex : p_getMapColorIndex_0_.equals("podzol") ? MapColor.obsidianColor.colorIndex : p_getMapColorIndex_0_.equals(
				"netherrack") ? MapColor.netherrackColor.colorIndex : !p_getMapColorIndex_0_.equals("snow") && !p_getMapColorIndex_0_.equals("white") ? !p_getMapColorIndex_0_.equals(
				"adobe") && !p_getMapColorIndex_0_.equals("orange") ? p_getMapColorIndex_0_.equals("magenta") ? MapColor.magentaColor.colorIndex : !p_getMapColorIndex_0_.equals(
				"light_blue") && !p_getMapColorIndex_0_.equals("lightBlue") ? p_getMapColorIndex_0_.equals("yellow") ? MapColor.yellowColor.colorIndex : p_getMapColorIndex_0_.equals(
				"lime") ? MapColor.limeColor.colorIndex : p_getMapColorIndex_0_.equals("pink") ? MapColor.pinkColor.colorIndex : p_getMapColorIndex_0_.equals(
				"gray") ? MapColor.grayColor.colorIndex : p_getMapColorIndex_0_.equals("silver") ? MapColor.silverColor.colorIndex : p_getMapColorIndex_0_.equals(
				"cyan") ? MapColor.cyanColor.colorIndex : p_getMapColorIndex_0_.equals("purple") ? MapColor.purpleColor.colorIndex : p_getMapColorIndex_0_.equals(
				"blue") ? MapColor.blueColor.colorIndex : p_getMapColorIndex_0_.equals("brown") ? MapColor.brownColor.colorIndex : p_getMapColorIndex_0_.equals(
				"green") ? MapColor.greenColor.colorIndex : p_getMapColorIndex_0_.equals("red") ? MapColor.redColor.colorIndex : p_getMapColorIndex_0_.equals(
				"black") ? MapColor.blackColor.colorIndex : -1 : MapColor.lightBlueColor.colorIndex : MapColor.adobeColor.colorIndex : MapColor.snowColor.colorIndex;
	}
	...
	private static void setMapColors(int[] p_setMapColors_0_) {
		if (p_setMapColors_0_ != null) {
			...
			if (flag)
				Minecraft.get().getTextureManager().reloadBannerTextures();
		}
	}
