...
public class GuiShaderOptions extends GuiScreenOF {
	...
	public GuiShaderOptions(GuiScreen guiscreen, String screenName) {
		this(guiscreen);
	...
	private String getButtonText(ShaderOption so, int btnWidth) {
		...
		MCFontRenderer fontrenderer = Config.getMinecraft().fontRenderer;
		...
		if (guibutton.enabled) {
			if (guibutton.id < 200 && guibutton instanceof GuiButtonShaderOption) {
				...
				if (shaderoption instanceof ShaderOptionScreen) {
					...
					GuiShaderOptions guishaderoptions = new GuiShaderOptions(this, s);
