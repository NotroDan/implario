...
public class UpdateTest {
    public static void main(String[] args) throws Exception{
        ...
        Decoder decoder = new FastDecoder(new byte[]{0, 0, 0, 0});
        Encoder encoder = new FastEncoder();
        Update.generateUpdate(oneJar, decoder, encoder, false);
        ECDSA cert = new ECDSA(384);
        JarFile path = new JarFile(testJar);
        oneJar.remove("lolkek");
        oneJar.add("lol", new byte[]{});
        Encoder newEnc = new FastEncoder();
        Update update = Update.generateUpdate(oneJar, new FastDecoder(encoder.generate()), newEnc, false);
