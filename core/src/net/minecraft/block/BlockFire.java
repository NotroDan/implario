...
public class BlockFire extends Block {
	...
	protected BlockFire() {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(AGE, 0).withProperty(FLIP, Boolean.FALSE).withProperty(ALT, Boolean.FALSE).withProperty(NORTH,
				...
				Boolean.FALSE).withProperty(UPPER,
				0));
	...
	protected boolean canDie(World worldIn, BlockPos pos) {
		return worldIn.canLightningStrike(pos) || worldIn.canLightningStrike(pos.west()) || worldIn.canLightningStrike(pos.east()) || worldIn.canLightningStrike(
				pos.north()) || worldIn.canLightningStrike(pos.south());
	}
	...
	public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		if (rand.nextInt(24) == 0) {
			worldIn.playSound((double) ((float) pos.getX() + 0.5F), (double) ((float) pos.getY() + 0.5F), (double) ((float) pos.getZ() + 0.5F), "fire.fire", 1.0F + rand.nextFloat(),
		...
		if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && !Blocks.fire.canCatchFire(worldIn, pos.down())) {
			if (Blocks.fire.canCatchFire(worldIn, pos.west())) {
				for (int j = 0; j < 2; ++j) {
					...
					worldIn.spawnParticle(ParticleType.SMOKE_LARGE, d3, d8, d13, 0.0D, 0.0D, 0.0D, new int[0]);
				}
			}
			...
			if (Blocks.fire.canCatchFire(worldIn, pos.east())) {
				for (int k = 0; k < 2; ++k) {
					...
					worldIn.spawnParticle(ParticleType.SMOKE_LARGE, d4, d9, d14, 0.0D, 0.0D, 0.0D, new int[0]);
				}
			}
			...
			if (Blocks.fire.canCatchFire(worldIn, pos.north())) {
				for (int l = 0; l < 2; ++l) {
					...
					worldIn.spawnParticle(ParticleType.SMOKE_LARGE, d5, d10, d15, 0.0D, 0.0D, 0.0D, new int[0]);
				}
			}
			...
			if (Blocks.fire.canCatchFire(worldIn, pos.south())) {
				for (int i1 = 0; i1 < 2; ++i1) {
					...
					worldIn.spawnParticle(ParticleType.SMOKE_LARGE, d6, d11, d16, 0.0D, 0.0D, 0.0D, new int[0]);
				}
			}
			...
			if (Blocks.fire.canCatchFire(worldIn, pos.up())) {
				for (int j1 = 0; j1 < 2; ++j1) {
					...
					worldIn.spawnParticle(ParticleType.SMOKE_LARGE, d7, d12, d17, 0.0D, 0.0D, 0.0D, new int[0]);
				}
			}
		} else {
			for (int i = 0; i < 3; ++i) {
				...
				worldIn.spawnParticle(ParticleType.SMOKE_LARGE, d0, d1, d2, 0.0D, 0.0D, 0.0D, new int[0]);
			}
		}
	}
