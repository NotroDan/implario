...
public class BlockSponge extends Block {
	...
	private boolean absorb(World worldIn, BlockPos pos) {
		...
		ArrayList<BlockPos> arraylist = new java.util.ArrayList<>();
	...
	public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		if (!state.getValue(WET)) return;
		EnumFacing enumfacing = EnumFacing.random(rand);

		if (enumfacing == EnumFacing.UP || World.doesBlockHaveSolidTopSurface(worldIn, pos.offset(enumfacing))) {return;}
		...
		if (enumfacing == EnumFacing.DOWN) {
			...
			if (enumfacing.getAxis() == EnumFacing.Axis.X) {
				d2 += rand.nextDouble();
				d0 += enumfacing == EnumFacing.EAST ? 1 : 0.05D;
			} else {
				d0 += rand.nextDouble();
				d2 += enumfacing == EnumFacing.SOUTH ? 1 : 0.05D;
			}
		}
