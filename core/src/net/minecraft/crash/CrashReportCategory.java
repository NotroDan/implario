...
public class CrashReportCategory {
	...
	public static String getCoordinateInfo(BlockPos pos) {
		...
		try {
			...
			int j5 = (j3 + 1 << 9) - 1;
			int i3 = (k3 + 1 << 9) - 1;
			stringbuilder.append(String.format("Region: (%d,%d; contains chunks %d,%d to %d,%d, blocks %d,0,%d to %d,255,%d)",
					j3, k3, l3, i4, j4, k4, l4, i5, j5, i3));
	...
	public static void addBlockInfo(CrashReportCategory category, final BlockPos pos, final Block blockIn, final int blockData) {
		...
		category.addCrashSectionCallable("Block data value", () -> {
			if (blockData < 0) return "Unknown? (Got " + blockData + ")";
	...
	public void appendToStringBuilder(StringBuilder builder) {
		...
		if (this.stackTrace != null && stackTrace.length > 0) {
			builder.append("\nStacktrace:");
