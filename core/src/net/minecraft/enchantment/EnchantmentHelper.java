...
public class EnchantmentHelper {
			...
			if (nbttaglist != null) {
				for (int i = 0; i < nbttaglist.tagCount(); ++i) {
					...
					if (Enchantment.getEnchantmentById(id) != null) {
						modifier.calculateModifier(Enchantment.getEnchantmentById(id), lvl);
					}
				}
			}
		}
	 ...
	 * WITHOUT RANDOM
	 */
	public static int getEnchantmentModifierDamageNoRandom(ItemStack[] stacks, DamageSource source) {
		enchantmentModifierDamage.damageModifier = 0;
		enchantmentModifierDamage.source = source;
		applyEnchantmentModifierArray(enchantmentModifierDamage, stacks);
