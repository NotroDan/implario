...
public class EnchantmentProtection extends Enchantment {
	...
	public int calcModifierDamage(int level, DamageSource source) {
		...
		if (this.protectionType == 0) return MathHelper.floor_float(f * 0.75F);
		if (this.protectionType == 1 && source.isFireDamage()) return MathHelper.floor_float(f * 1.25F);
		if (this.protectionType == 2 && source == DamageSource.fall) return MathHelper.floor_float(
				f * 2.5F);
		if (this.protectionType == 3 && source.isExplosion()) return MathHelper.floor_float(f * 1.5F);
		if (this.protectionType == 4 && source.isProjectile()) return MathHelper.floor_float(
				f * 1.5F);
		return 0;
	}
