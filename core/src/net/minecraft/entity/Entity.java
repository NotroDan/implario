...
public abstract class Entity implements Sender, ITrackable {
	...
	public void onEntityUpdate() {
			...
			if (this.inPortal) {
				if (minecraftserver.getAllowNether()) {
					if (this.ridingEntity == null && this.portalCounter++ >= i) {
						...
						if (this.worldObj.provider.getDimensionId() == -1) j = 0;
						else j = -1;
		...
		if (this.worldObj.isClientSide) this.fire = 0;
			...
			if (this.isImmuneToFire) {
				...
				if (this.fire % 20 == 0) this.attackEntityFrom(DamageSource.onFire, 1.0F);
				--this.fire;
			}
		}
...
//		this.onGround = true;
	...
	protected void updateFallState(double y, boolean onGroundIn, Block blockIn, BlockPos pos) {
		if (onGroundIn) {
			if (this.fallDistance <= 0.0F) return;
			if (blockIn != null) blockIn.onFallenUpon(this.worldObj, pos, this, this.fallDistance);
			else this.fall(this.fallDistance, 1.0F);
			this.fallDistance = 0.0F;
		} else if (y < 0.0D) this.fallDistance = (float) ((double) this.fallDistance - y);
	}
	...
	public boolean isWet() {
		return this.inWater || this.worldObj.canLightningStrike(new BlockPos(this.posX, this.posY, this.posZ)) || this.worldObj.canLightningStrike(
				new BlockPos(this.posX, this.posY + (double) this.height, this.posZ));
	}
	...
	public void move(float strafe, float forward, float friction) {
		...
		this.motionX += (double) (strafe * dz - forward * dx);
		this.motionZ += (double) (forward * dz + strafe * dx);
	}
	...
	public void mountEntity(Entity entityIn) {
		...
		if (entityIn == null) {
			if (this.ridingEntity != null) {
				this.setLocationAndAngles(this.ridingEntity.posX, this.ridingEntity.getEntityBoundingBox().minY + (double) this.ridingEntity.height, this.ridingEntity.posZ, this.rotationYaw,
						this.rotationPitch);
	...
	public void func_181015_d(BlockPos p_181015_1_) {
		if (this.timeUntilPortal > 0) {
			...
			if (!this.worldObj.isClientSide && !p_181015_1_.equals(this.field_181016_an)) {
				...
				d1 = Math.abs(MathHelper.func_181160_c(d1 - (double) (blockpattern$patternhelper.getFinger().rotateY().getAxisDirection() == EnumFacing.AxisDirection.NEGATIVE ? 1 : 0), d0,
	...
	public String toString() {
		return String.format("%s[\'%s\'/%d, l=\'%s\', x=%.2f, y=%.2f, z=%.2f]", this.getClass().getSimpleName(), this.getName(),
				this.entityId, this.worldObj == null ? "~NULL~" : this.worldObj.getWorldInfo().getWorldName(),
				this.posX,
				this.posY,
				this.posZ);
	}
	...
	public void travelToDimension(int destDim) {
		if (this.worldObj.isClientSide || this.isDead) return;
		this.worldObj.theProfiler.startSection("changeDimension");
		MinecraftServer server = MinecraftServer.getServer();
		int fromDim = this.dimension;
		WorldServer fromWorld = server.worldServerForDimension(fromDim);
		WorldServer destWorld = server.worldServerForDimension(destDim);
		this.dimension = destDim;

		if (fromDim == 1 && destDim == 1) {
			// Портал в Энд внутри Энда должен телепортировать в обычный мир.
			destWorld = server.worldServerForDimension(0);
			this.dimension = 0;
		}
		...
		server.getConfigurationManager().transferEntityToWorld(this, fromDim, fromWorld, destWorld);
		this.worldObj.theProfiler.endStartSection("reloading");
		Entity entity = EntityList.createEntityByName(EntityList.getEntityString(this), destWorld);

		if (entity != null) {
			entity.copyDataFromOld(this);

			if (fromDim == 1 && destDim == 1) {
				BlockPos blockpos = this.worldObj.getTopSolidOrLiquidBlock(destWorld.getSpawnPoint());
	...
	public void addEntityCrashInfo(CrashReportCategory category) {
		...
		category.addCrashSection("Entity\'s Block location",
				CrashReportCategory.getCoordinateInfo((double) MathHelper.floor_double(this.posX), (double) MathHelper.floor_double(this.posY), (double) MathHelper.floor_double(this.posZ)));
