...
public abstract class EntityLivingBase extends Entity {
		...
		if (i > 0) {
			...
			if (flag && i > 0) {
				...
				this.worldObj.spawnParticle(flag1 ? ParticleType.SPELL_MOB_AMBIENT : ParticleType.SPELL_MOB, this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width,
						this.posY + this.rand.nextDouble() * (double) this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.width, d0, d1, d2);
			}
		}
	}
	...
	private int getArmSwingAnimationEnd() {
		return this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : this.isPotionActive(
				Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6;
	}
			...
			for (int k = -i; k < i; ++k) {
				if (j != 0 || k != 0) {
					...
					if (this.worldObj.func_147461_a(axisalignedbb).isEmpty()) {
						...
						if (World.doesBlockHaveSolidTopSurface(this.worldObj, new BlockPos(l, (int) this.posY - 1, i1)) || this.worldObj.getBlockState(
								new BlockPos(l, (int) this.posY - 1, i1)).getBlock().getMaterial() == Material.water) {
		...
		if (this.isServerWorld()) {
			if (!this.isInWater() || this instanceof Player && ((Player) this).capabilities.isFlying) {
				if (!this.isInLava() || this instanceof Player && ((Player) this).capabilities.isFlying) {
					...
					// Скользкость
					if (this.onGround) {
						slip = this.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.getEntityBoundingBox().minY) - 1,
								MathHelper.floor_double(this.posZ))).getBlock().slipperiness * 0.91F;
					}

					float motion = 0.16277136F / (slip * slip * slip);
					motion = this.onGround ? this.getAIMoveSpeed() * motion : this.jumpMovementFactor;

					this.move(strafe, forward, motion);
					...
					double d1 = this.posY;
					this.move(strafe, forward, 0.02F);
					this.moveEntity(this.motionX, this.motionY, this.motionZ);
					this.motionX *= 0.5D;
					this.motionY *= 0.5D;
					this.motionZ *= 0.5D;
