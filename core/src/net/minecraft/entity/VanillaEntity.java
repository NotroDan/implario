...
public abstract class VanillaEntity extends EntityLivingBase {
	...
	public void spawnExplosionParticle() {
		if (this.worldObj.isRemote) {
			for (int i = 0; i < 20; ++i) {
				...
				double d3 = 10.0D;
				this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width - d0 * d3,
						this.posY + (double) (this.rand.nextFloat() * this.height) - d1 * d3, this.posZ + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width - d2 * d3, d0, d1,
						d2);
			}
		} else {
	...
	public void onUpdate() {
		if (Todo.instance.isSmoothWorld() && this.canSkipUpdate()) {
	...
	protected void dropFewItems(boolean p_70628_1_, int p_70628_2_) {
		...
		if (p_70628_2_ > 0) i += this.rand.nextInt(p_70628_2_ + 1);
		for (int j = 0; j < i; ++j) this.dropItem(item, 1);
	}
	...
	public void writeEntityToNBT(NBTTagCompound tagCompound) {
		...
		for (ItemStack anEquipment : this.equipment) {
			...
			if (anEquipment != null) {
				anEquipment.writeToNBT(nbttagcompound);
			}
	...
	protected void updateEquipmentIfNeeded(EntityItem itemEntity) {
		...
		if (i > -1) {
			...
			if (flag && this.canPickItemUp(itemstack)) {
	...
	public boolean isNotColliding() {
		return this.worldObj.checkNoEntityCollision(this.getEntityBoundingBox(), this) && this.worldObj.getCollidingBoundingBoxes(this,
				this.getEntityBoundingBox()).isEmpty() && !this.worldObj.isAnyLiquid(this.getEntityBoundingBox());
	}
