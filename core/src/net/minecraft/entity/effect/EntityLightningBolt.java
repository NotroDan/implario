...
public class EntityLightningBolt extends EntityWeatherEffect {
	...
	public EntityLightningBolt(World worldIn, double posX, double posY, double posZ) {
		...
		if (!worldIn.isClientSide && worldIn.getGameRules().getBoolean(
				"doFireTick") && (worldIn.getDifficulty() == EnumDifficulty.NORMAL || worldIn.getDifficulty() == EnumDifficulty.HARD) && worldIn.isAreaLoaded(blockpos, 10)) {
	...
	public void onUpdate() {
		...
		if (this.lightningState < 0) {
			if (this.boltLivingTime == 0) {
				...
				if (!this.worldObj.isClientSide && this.worldObj.getGameRules().getBoolean("doFireTick") && this.worldObj.isAreaLoaded(blockpos, 10) && this.worldObj.getBlockState(
						blockpos).getBlock().getMaterial() == Material.air && Blocks.fire.canPlaceBlockAt(this.worldObj, blockpos)) {
