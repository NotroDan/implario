...
public class EntityArrow extends Entity implements IProjectile {
	...
	public void onUpdate() {
		...
		if (this.inGround) {
			...
			MovingObjectPosition movingobjectposition = this.worldObj.rayTraceBlocks(vec31D, vec3D, false, true, false);
					...
					if (movingobjectposition.entityHit.attackEntityFrom(damagesource, (float) l)) {
						if (movingobjectposition.entityHit instanceof EntityLivingBase) {
							...
							if (this.knockbackStrength > 0) {
								...
								if (f7 > 0.0F) {
									movingobjectposition.entityHit.addVelocity(this.motionX * (double) this.knockbackStrength * 0.6000000238418579D / (double) f7, 0.1D,
							...
							if (this.shootingEntity != null && movingobjectposition.entityHit != this.shootingEntity && movingobjectposition.entityHit instanceof Player && this.shootingEntity instanceof MPlayer) {
								((MPlayer) this.shootingEntity).playerNetServerHandler.sendPacket(new S2BPacketChangeGameState(6, 0.0F));
							}
						}
						...
						this.motionX *= -0x1.999999999999ap-4;
			...
			if (this.getIsCritical()) {
				for (int k = 0; k < 4; ++k) {
					this.worldObj.spawnParticle(ParticleType.CRIT, this.posX + this.motionX * (double) k / 4.0D, this.posY + this.motionY * (double) k / 4.0D,
							this.posZ + this.motionZ * (double) k / 4.0D, -this.motionX, -this.motionY + 0.2D, -this.motionZ, new int[0]);
				}
			}
			...
			if (this.isInWater()) {
				for (int i1 = 0; i1 < 4; ++i1) {
					float f8 = 0.25F;
					this.worldObj.spawnParticle(ParticleType.WATER_BUBBLE, this.posX - this.motionX * (double) f8, this.posY - this.motionY * (double) f8, this.posZ - this.motionZ * (double) f8,
