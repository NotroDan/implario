...
public abstract class Container {
	...
	public void detectAndSendChanges() {
		for (int i = 0; i < this.inventorySlots.size(); ++i) {
			...
			if (!ItemStack.areItemStacksEqual(itemstack1, itemstack)) {
				...
				for (int j = 0; j < this.crafters.size(); ++j) this.crafters.get(j).sendSlotContents(this, i, itemstack1);
			}
		}
	}
	...
	public Slot getSlotFromInventory(Inventory inv, int slotIn) {
		for (int i = 0; i < this.inventorySlots.size(); ++i) {
			...
			if (slot.isHere(inv, slotIn)) return slot;
		}
	...
	public ItemStack slotClick(int slotId, int clickedButton, ClickType mode, Player playerIn) {
		...
		if (mode == DRAG) {
			...
			if ((i != 1 || this.dragEvent != 2) && i != this.dragEvent) this.resetDrag();
				...
				if (slot != null && canAddItemToSlot(slot, inventoryplayer.getItemStack(), true) && slot.isItemValid(
						inventoryplayer.getItemStack()) && inventoryplayer.getItemStack().stackSize > this.dragSlots.size() && this.canDragIntoSlot(slot)) this.dragSlots.add(slot);
				...
				if (!this.dragSlots.isEmpty()) {
					...
					for (Slot slot1 : this.dragSlots)
						if (slot1 != null && canAddItemToSlot(slot1, inventoryplayer.getItemStack(), true) && slot1.isItemValid(
								inventoryplayer.getItemStack()) && inventoryplayer.getItemStack().stackSize >= this.dragSlots.size() && this.canDragIntoSlot(slot1)) {
							ItemStack itemstack1 = itemstack3.copy();
							int k = slot1.getHasStack() ? slot1.getStack().stackSize : 0;
							computeStackSize(this.dragSlots, this.dragMode, itemstack1, k);

							if (itemstack1.stackSize > itemstack1.getMaxStackSize()) itemstack1.stackSize = itemstack1.getMaxStackSize();

							if (itemstack1.stackSize > slot1.getItemStackLimit(itemstack1)) itemstack1.stackSize = slot1.getItemStackLimit(itemstack1);
			...
			if (slot6 != null && slot6.canTakeStack(playerIn)) {
				...
				if (itemstack8 != null) {
					...
					if (slot6.getStack() != null && slot6.getStack().getItem() == item) this.retrySlotClick(slotId, clickedButton, true, playerIn);
				}
			}
		} else {
			if (slotId < 0) return null;
			...
			if (slot7 != null) {
				...
				if (itemstack9 == null) {
					...
					if (itemstack9.getItem() == itemstack10.getItem() && itemstack9.getMetadata() == itemstack10.getMetadata() && ItemStack.areItemStackTagsEqual(itemstack9, itemstack10)) {
						...
						if (i2 > slot7.getItemStackLimit(itemstack10) - itemstack9.stackSize) i2 = slot7.getItemStackLimit(itemstack10) - itemstack9.stackSize;

						if (i2 > itemstack10.getMaxStackSize() - itemstack9.stackSize) i2 = itemstack10.getMaxStackSize() - itemstack9.stackSize;
				...
				} else if (itemstack9.getItem() == itemstack10.getItem() && itemstack10.getMaxStackSize() > 1 && (!itemstack9.getHasSubtypes() || itemstack9.getMetadata() == itemstack10.getMetadata()) && ItemStack.areItemStackTagsEqual(
		...
		else if (mode == HOTBAR && clickedButton >= 0 && clickedButton < 9) {
		...
		} else if (mode == PICK && playerIn.capabilities.isCreativeMode && inventoryplayer.getItemStack() == null && slotId >= 0) {
		...
		} else if (mode == DROP && inventoryplayer.getItemStack() == null && slotId >= 0) {
			...
			if (itemstack4 != null && (slot2 == null || !slot2.getHasStack() || !slot2.canTakeStack(playerIn))) {
				...
				for (int l2 = 0; l2 < 2; ++l2)
					for (int i3 = i1; i3 >= 0 && i3 < this.inventorySlots.size() && itemstack4.stackSize < itemstack4.getMaxStackSize(); i3 += j1) {
						...
						if (slot8.getHasStack() && canAddItemToSlot(slot8, itemstack4, true) && slot8.canTakeStack(playerIn) && this.canMergeSlot(itemstack4,
								slot8) && (l2 != 0 || slot8.getStack().stackSize != slot8.getStack().getMaxStackSize())) {
	...
	protected boolean mergeItemStack(ItemStack stack, int startIndex, int endIndex, boolean reverseDirection) {
		...
		if (stack.isStackable()) while (stack.stackSize > 0 && (!reverseDirection && i < endIndex || reverseDirection && i >= startIndex)) {
			Slot slot = this.inventorySlots.get(i);
			ItemStack itemstack = slot.getStack();

			if (itemstack != null && itemstack.getItem() == stack.getItem() && (!stack.getHasSubtypes() || stack.getMetadata() == itemstack.getMetadata()) && ItemStack.areItemStackTagsEqual(stack,
					itemstack)) {
