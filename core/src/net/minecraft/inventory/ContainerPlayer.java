		...
		for (int k = 0; k < 4; ++k) {
			...
			this.addSlotToContainer(new Slot(playerInventory, playerInventory.getSizeInventory() - 1 - k, 8, 8 + k * 18) {
				...
				public boolean isItemValid(ItemStack stack) {
					return stack != null && (stack.getItem() instanceof ItemArmor ? ((ItemArmor) stack.getItem()).armorType == f : (stack.getItem() == Item.getItemFromBlock(
							Blocks.pumpkin) || stack.getItem() == Items.skull) && f == 0);
				}

				public String getSlotTexture() {
					return ItemArmor.EMPTY_SLOT_NAMES[f];
				}
			});
		}
