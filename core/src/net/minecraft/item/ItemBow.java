...
public class ItemBow extends Item {
	...
	public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityPlayer playerIn, int timeLeft) {
		...
		int j = EnchantmentHelper.getEnchantmentLevel(Enchantments.power.effectId, stack);

		if (j > 0) entityarrow.setDamage(entityarrow.getDamage() + (double) j * 0.5D + 0.5D);

		int k = EnchantmentHelper.getEnchantmentLevel(Enchantments.punch.effectId, stack);

		if (k > 0) entityarrow.setKnockbackStrength(k);

		if (EnchantmentHelper.getEnchantmentLevel(Enchantments.flame.effectId, stack) > 0) entityarrow.setFire(100);
