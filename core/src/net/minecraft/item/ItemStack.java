...
public final class ItemStack {
	...
	public ItemStack splitStack(int amount) {
		...
		if (this.stackTagCompound != null) itemstack.stackTagCompound = (NBTTagCompound) this.stackTagCompound.copy();
	 ...
	 */
	public boolean onItemUse(Player playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
	 ...
	 */
	public ItemStack useItemRightClick(World worldIn, Player playerIn) {
	 ...
	 */
	public ItemStack onItemUseFinish(World worldIn, Player playerIn) {
	...
	public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
		...
		if (this.stackTagCompound != null) nbt.setTag("tag", this.stackTagCompound);
	...
	public boolean attemptDamageItem(int amount, Random rand) {
		...
		if (amount > 0) {
			int i = EnchantmentHelper.getEnchantmentLevel(Enchantments.unbreaking.effectId, this);
			int j = 0;

			for (int k = 0; i > 0 && k < amount; ++k) if (EnchantmentDurability.negateDamage(this, i, rand)) ++j;
	...
	public void damageItem(int amount, EntityLivingBase entityIn) {
		if (!(entityIn instanceof Player) || !((Player) entityIn).capabilities.isCreativeMode) if (this.isItemStackDamageable()) if (this.attemptDamageItem(amount, entityIn.getRNG())) {
	 ...
	 */
	public void hitEntity(EntityLivingBase entityIn, Player playerIn) {
	 ...
	 */
	public void onBlockDestroyed(World worldIn, Block blockIn, BlockPos pos, Player playerIn) {
	...
	public boolean interactWithEntity(Player playerIn, EntityLivingBase entityIn) {
	 ...
	 */
	public void onPlayerStoppedUsing(World worldIn, Player playerIn, int timeLeft) {
	...
	public NBTTagCompound getSubCompound(String key, boolean create) {
		if (this.stackTagCompound != null && this.stackTagCompound.hasKey(key, 10)) return this.stackTagCompound.getCompoundTag(key);
		if (create) {
	...
	public String getDisplayName() {
		...
		if (this.stackTagCompound != null && this.stackTagCompound.hasKey("display", 10)) {
			...
			if (nbttagcompound.hasKey("Name", 8)) s = nbttagcompound.getString("Name");
		}
	...
	public ItemStack setStackDisplayName(String displayName) {
		if (this.stackTagCompound == null) this.stackTagCompound = new NBTTagCompound();

		if (!this.stackTagCompound.hasKey("display", 10)) this.stackTagCompound.setTag("display", new NBTTagCompound());
	...
	public void clearCustomName() {
		if (this.stackTagCompound != null) if (this.stackTagCompound.hasKey("display", 10)) {
			NBTTagCompound nbttagcompound = this.stackTagCompound.getCompoundTag("display");
	...
	public List<String> getTooltip(Player playerIn, boolean advanced) {
		...
		if (!this.hasDisplayName() && this.item == Items.filled_map) s = s + " #" + this.itemDamage;

		list.add(s);
		int i1 = 0;

		if (this.hasTagCompound() && this.stackTagCompound.hasKey("HideFlags", 99)) i1 = this.stackTagCompound.getInteger("HideFlags");
		...
		if (this.hasTagCompound()) {
			if ((i1 & 1) == 0) {
				...
				if (nbttaglist != null) for (int j = 0; j < nbttaglist.tagCount(); ++j) {
					int k = nbttaglist.getCompoundTagAt(j).getShort("id");
					int l = nbttaglist.getCompoundTagAt(j).getShort("lvl");

					if (Enchantment.getEnchantmentById(k) != null) list.add(Enchantment.getEnchantmentById(k).getTranslatedName(l));
				}
			}
			...
			if (this.stackTagCompound.hasKey("display", 10)) {
				...
				if (nbttagcompound.hasKey("color", 3)) if (advanced) list.add("Color: #" + Integer.toHexString(nbttagcompound.getInteger("color")).toUpperCase());
				else list.add(EnumChatFormatting.ITALIC + StatCollector.translateToLocal("item.dyed"));

				if (nbttagcompound.getTagId("Lore") == 9) {
					NBTTagList nbttaglist1 = nbttagcompound.getTagList("Lore", 8);

					if (nbttaglist1.tagCount() > 0)
						for (int j1 = 0; j1 < nbttaglist1.tagCount(); ++j1) list.add(EnumChatFormatting.DARK_PURPLE + "" + EnumChatFormatting.ITALIC + nbttaglist1.getStringTagAt(j1));
				}
			}
		}
		...
		if (!multimap.isEmpty() && (i1 & 2) == 0) {
			...
			for (Entry<String, AttributeModifier> entry : multimap.entries()) {
				...
				if (attributemodifier.getOperation() != 1 && attributemodifier.getOperation() != 2) d1 = d0;
				else d1 = d0 * 100.0D;

				if (d0 > 0.0D) list.add(EnumChatFormatting.BLUE + StatCollector.translateToLocalFormatted("attribute.modifier.plus." + attributemodifier.getOperation(),
						new Object[] {DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + entry.getKey())}));
				else if (d0 < 0.0D) {
					d1 = d1 * -1.0D;
					list.add(EnumChatFormatting.RED + StatCollector.translateToLocalFormatted("attribute.modifier.take." + attributemodifier.getOperation(),
							new Object[] {DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + entry.getKey())}));
				}
			}
		}

		if (this.hasTagCompound() && this.getTagCompound().getBoolean("Unbreakable") && (i1 & 4) == 0) list.add(EnumChatFormatting.BLUE + StatCollector.translateToLocal("item.unbreakable"));
		...
		if (advanced) {
			...
			if (this.hasTagCompound()) list.add(EnumChatFormatting.DARK_GRAY + "NBT: " + this.getTagCompound().getKeySet().size() + " tag(s)");
		}
	...
	public void setRepairCost(int cost) {
		if (!this.hasTagCompound()) this.stackTagCompound = new NBTTagCompound();
	...
	public IChatComponent getChatComponent() {
		...
		if (this.hasDisplayName()) chatcomponenttext.getChatStyle().setItalic(Boolean.TRUE);
	...
	public boolean canDestroy(Block blockIn) {
		if (blockIn == this.canDestroyCacheBlock) return this.canDestroyCacheResult;
	...
	public boolean canPlaceOn(Block blockIn) {
		if (blockIn == this.canPlaceOnCacheBlock) return this.canPlaceOnCacheResult;
