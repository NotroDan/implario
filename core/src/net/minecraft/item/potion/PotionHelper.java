...
public class PotionHelper {
	...
	private static final String[] potionPrefixes = new String[] {
			"potion.prefix.mundane", "potion.prefix.uninteresting", "potion.prefix.bland", "potion.prefix.clear", "potion.prefix.milky", "potion.prefix.diffuse", "potion.prefix.artless",
			"potion.prefix.thin", "potion.prefix.awkward", "potion.prefix.flat", "potion.prefix.bulky", "potion.prefix.bungling", "potion.prefix.buttered", "potion.prefix.smooth",
			"potion.prefix.suave", "potion.prefix.debonair", "potion.prefix.thick", "potion.prefix.elegant", "potion.prefix.fancy", "potion.prefix.charming", "potion.prefix.dashing",
	 ...
	 */
	public static List<PotionEffect> getPotionEffects(int a, boolean b) {
		ArrayList<PotionEffect> arraylist = null;

		for (Potion potion : Potion.potionTypes) {
			if (potion == null || potion.isUsable() && !b) continue;
			String s = (String) potionRequirements.get(potion.getId());

			if (s == null) continue;
			int i = parsePotionEffects(s, 0, s.length(), a);
			...
			if (s1 != null) {
				j = parsePotionEffects(s1, 0, s1.length(), a);
				if (j < 0) j = 0;
			}

			if (potion.isInstant()) i = 1;
			else {
				i = 1200 * (i * 3 + (i - 1) * 2);
				i = i >> j;
				i = (int) Math.round((double) i * potion.getEffectiveness());

				if ((a & 0b100_0000_0000_0000) != 0)
	...
	public static int func_77908_a(int p_77908_0_, int p_77908_1_, int p_77908_2_, int p_77908_3_, int p_77908_4_, int p_77908_5_) {
		return (checkFlag(p_77908_0_, p_77908_1_) ? 16 : 0) | (checkFlag(p_77908_0_, p_77908_2_) ? 8 : 0) | (checkFlag(p_77908_0_, p_77908_3_) ? 4 : 0) | (checkFlag(p_77908_0_,
				p_77908_4_) ? 2 : 0) | (checkFlag(p_77908_0_, p_77908_5_) ? 1 : 0);
	}
