...
public class S21PacketChunkData implements Packet<INetHandlerPlayClient> {
	...
	public S21PacketChunkData(Chunk chunkIn, boolean p_i45196_2_, int p_i45196_3_) {
		...
		this.extractedData = encdode(chunkIn, p_i45196_2_, !chunkIn.getWorld().provider.getHasNoSky(), p_i45196_3_);
	}
...
//		System.out.println(getData().length + ", x=" + getChunkX() + ", z=" + getChunkZ() + ", " + Textifier.bytesToHex(getData()));
	...
	public static S21PacketChunkData.Extracted encdode(Chunk chunk, boolean empty, boolean p_179756_2_, int p_179756_3_) {
		...
		for (int i = 0; i < storages.length; ++i) {
			...
			if (extendedblockstorage != null && !(empty && extendedblockstorage.isEmpty()) && (p_179756_3_ & 1 << i) != 0) {
				res.dataSize |= 1 << i;
				list.add(extendedblockstorage);
			}
		}

		res.data = new byte[func_180737_a(Integer.bitCount(res.dataSize), p_179756_2_, empty)];
		int j = 0;
		...
		for (ExtendedBlockStorage extendedblockstorage1 : list) {
			...
			for (char c0 : achar) {
				res.data[j++] = (byte) (c0 & 255);
				res.data[j++] = (byte) (c0 >> 8 & 255);
			}
		}

		for (ExtendedBlockStorage extendedblockstorage2 : list) {
			j = func_179757_a(extendedblockstorage2.getBlocklightArray().getData(), res.data, j);
		}

		if (p_179756_2_) {
			for (ExtendedBlockStorage extendedblockstorage3 : list) {
				j = func_179757_a(extendedblockstorage3.getSkylightArray().getData(), res.data, j);
			}
		}

		if (empty) {
			func_179757_a(chunk.getBiomeArray(), res.data, j);
		}
