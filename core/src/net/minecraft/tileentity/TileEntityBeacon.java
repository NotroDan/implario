...
public class TileEntityBeacon extends TileEntityLockable implements ITickable, IInventory {
	...
	private void addEffectsToPlayers() {
		if (this.isComplete && this.levels > 0 && !this.worldObj.isClientSide && this.primaryEffect > 0) {
			...
			int k = this.pos.getY();
			int l = this.pos.getZ();
			AxisAlignedBB axisalignedbb = new AxisAlignedBB((double) j, (double) k, (double) l, (double) (j + 1), (double) (k + 1), (double) (l + 1)).expand(d0, d0, d0).addCoord(0.0D,
					(double) this.worldObj.getHeight(), 0.0D);
			List<Player> list = this.worldObj.getEntitiesWithinAABB(Player.class, axisalignedbb);
	...
	private void updateSegmentColors() {
		...
		TileEntityBeacon.BeamSegment s = new TileEntityBeacon.BeamSegment(BeaconColors.getColor(EnumDyeColor.WHITE));
		this.beamSegments.add(s);
		boolean flag = true;
		...
		for (int i1 = k + 1; i1 < 256; ++i1) {
			...
			if (iblockstate.getBlock() == Blocks.stained_glass) {
				afloat = BeaconColors.getColor(iblockstate.getValue(BlockStainedGlass.COLOR));
			} else {
				...
				afloat = BeaconColors.getColor(iblockstate.getValue(BlockStainedGlassPane.COLOR));
			}

			if (!flag) {
				afloat = new float[] {
						(s.getColors()[0] + afloat[0]) / 2.0F, (s.getColors()[1] + afloat[1]) / 2.0F,
						(s.getColors()[2] + afloat[2]) / 2.0F
				};
			}

			if (Arrays.equals(afloat, s.getColors())) {
				s.incrementHeight();
			} else {
				s = new TileEntityBeacon.BeamSegment(afloat);
				this.beamSegments.add(s);
			}
		...
		if (!this.worldObj.isClientSide && this.levels == 4 && i < this.levels) {
			for (Player entityplayer : this.worldObj.getEntitiesWithinAABB(Player.class,
