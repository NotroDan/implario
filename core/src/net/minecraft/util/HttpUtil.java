...
public class HttpUtil {
	...
	public static final ListeningExecutorService field_180193_a = MoreExecutors.listeningDecorator(Executors.newCachedThreadPool(
			new ThreadFactoryBuilder().setDaemon(true).setNameFormat("Downloader %d").build()));
	...
	private static String post(URL url, String content, boolean skipLoggingErrors) {
		try {
			...
			if (!skipLoggingErrors) logger.error("Could not post to " + url, exception);

			return "";
		}
	}

	public static ListenableFuture<Object> downloadResourcePack(final File saveFile, final String packUrl, final Map<String, String> playerInfo, final int maxSize, final IProgressUpdate p_180192_4_,
			...
			public void run() {
				...
				try {
					try {
						...
						float f = 0.0F;
						float f1 = (float) playerInfo.entrySet().size();

						for (Entry<String, String> entry : playerInfo.entrySet()) {
							httpurlconnection.setRequestProperty(entry.getKey(), entry.getValue());

							if (p_180192_4_ != null) p_180192_4_.setLoadingProgress((int) (++f / f1 * 100.0F));
						}
						...
						if (p_180192_4_ != null) p_180192_4_.displayLoadingString(String.format("Downloading file (%.2f MB)...", f1 / 1000.0F / 1000.0F));

						if (saveFile.exists()) {
							long j = saveFile.length();

							if (j == (long) i) {
								if (p_180192_4_ != null) p_180192_4_.setDoneWorking();
						...
						} else if (saveFile.getParentFile() != null) saveFile.getParentFile().mkdirs();
