...
public interface TokenMap<N, T> {
	 ...
	 * @return a {@code TokenMap}
	 */
	static <N, T> TokenMap<N, T> of(Map<Key<N, ? extends T>, T> map) {
	 ...
	 * if this map contains no mapping for the name and type
	 */
	<U extends T> U get(N name, Class<U> type);
	 ...
	 */
	default <U extends T> U put(N name, Class<U> type, U value) {
	 ...
	 */
	default <U extends T> U put(Key<N, U> key, U value) {
	 ...
	 */
	<U extends T> U remove(N name, Class<U> type);
	 ...
	 * was no mapping for the key
	 */
	default <U extends T> U remove(Key<N, U> key) {
...
class HashTokenMap<N, T> extends HashMap<Key<N, ? extends T>, T> implements TokenMap<N, T> {
	...
	public <U extends T> U getOrDefault(N name, Class<U> type, U value) {
		Key<N, ? extends T> key = cached.set(name, type);
		return (U) super.getOrDefault(key, value);
	}
