...
public class Explosion {
	...
	public Explosion(World worldIn, Entity p_i45753_2_, double p_i45753_3_, double p_i45753_5_, double p_i45753_7_, float p_i45753_9_, boolean p_i45753_10_, boolean p_i45753_11_,
					...
					if (j == 0 || j == 15 || k == 0 || k == 15 || l == 0 || l == 15) {
						...
						for (float f1 = 0.3F; f > 0.0F; f -= 0.22500001F) {
							...
							if (iblockstate.getBlock().getMaterial() != Material.air) {
								float f2 = this.exploder != null ? this.exploder.getExplosionResistance(this, this.worldObj, blockpos, iblockstate) : iblockstate.getBlock().getExplosionResistance(
										(Entity) null);
								f -= (f2 + 0.3F) * 0.3F;
							}
	...
	public void doExplosionB(boolean spawnParticles) {
		...
		if (this.explosionSize >= 2.0F && this.isSmoking) {
			this.worldObj.spawnParticle(ParticleType.EXPLOSION_HUGE, this.explosionX, this.explosionY, this.explosionZ, 1.0D, 0.0D, 0.0D, new int[0]);
		} else {
			this.worldObj.spawnParticle(ParticleType.EXPLOSION_LARGE, this.explosionX, this.explosionY, this.explosionZ, 1.0D, 0.0D, 0.0D, new int[0]);
		}
			...
			for (BlockPos blockpos : this.affectedBlockPositions) {
				...
				if (spawnParticles) {
					...
					this.worldObj.spawnParticle(ParticleType.EXPLOSION_NORMAL, (d0 + this.explosionX * 1.0D) / 2.0D, (d1 + this.explosionY * 1.0D) / 2.0D, (d2 + this.explosionZ * 1.0D) / 2.0D,
							d3, d4, d5, new int[0]);
					this.worldObj.spawnParticle(ParticleType.SMOKE_NORMAL, d0, d1, d2, d3, d4, d5, new int[0]);
				}
		...
		if (this.isFlaming) {
			for (BlockPos blockpos1 : this.affectedBlockPositions) {
				if (this.worldObj.getBlockState(blockpos1).getBlock().getMaterial() == Material.air && this.worldObj.getBlockState(
						blockpos1.down()).getBlock().isFullBlock() && this.explosionRNG.nextInt(3) == 0) {
