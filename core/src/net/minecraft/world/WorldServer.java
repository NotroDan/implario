...
public class WorldServer extends World {
	...
	public void tick() {
		...
		if(ServerEvents.worldTick.isUseful())
			ServerEvents.worldTick.call(tickEvent);
	...
	protected BlockPos adjustPosToNearbyEntity(BlockPos pos) {
		...
		List<EntityLivingBase> list = this.getEntitiesWithinAABB(EntityLivingBase.class, axisalignedbb,
				e -> e != null && e.isEntityAlive() && WorldServer.this.canSeeSky(e.getPosition()));
	...
	public boolean tickUpdates(boolean flag) {
		if (this.worldInfo.getTerrainType() == WorldType.DEBUG) return false;
			...
			for (BlockEventData blockeventdata : this.serverBlockEvents[i]) {
				if (this.fireBlockEvent(blockeventdata)) {
					this.mcServer.getConfigurationManager().sendToAllNear((double) blockeventdata.getPosition().getX(), (double) blockeventdata.getPosition().getY(),
							...
							new S24PacketBlockAction(blockeventdata.getPosition(), blockeventdata.getBlock(), blockeventdata.getEventID(), blockeventdata.getEventParameter()));
				}
			}
