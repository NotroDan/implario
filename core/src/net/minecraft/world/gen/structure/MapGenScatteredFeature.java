...
public class MapGenScatteredFeature extends MapGenStructure {
	...
	protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
		...
		if (i == k && j == l) {
			BiomeGenBase biomegenbase = this.worldObj.getWorldChunkManager().getBiomeGenerator(new BlockPos(i * 16 + 8, 0, j * 16 + 8)).toGenBase();
	...
	public static class Start extends StructureStart {
		...
		public Start(World worldIn, Random p_i2060_2_, int p_i2060_3_, int p_i2060_4_) {
			...
			BiomeGenBase biomegenbase = worldIn.getBiomeGenForCoords(new BlockPos(p_i2060_3_ * 16 + 8, 0, p_i2060_4_ * 16 + 8)).toGenBase();
