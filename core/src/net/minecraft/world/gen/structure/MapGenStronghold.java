...
public class MapGenStronghold extends MapGenStructure {
	...
	private List<Biome> field_151546_e;
	...
	public MapGenStronghold() {
		...
		for (Biome biome : Biome.biomeList) {
			if (biome instanceof BiomeGenBase) {
				BiomeGenBase gen = (BiomeGenBase) biome;
				if (gen.minHeight > 0.0F)
					this.field_151546_e.add(gen);
			}
		}
	}
	...
	protected StructureStart getStructureStart(int chunkX, int chunkZ) {
		...
		for (mapgenstronghold$start = new MapGenStronghold.Start(this.worldObj, this.rand, chunkX,
				chunkZ); mapgenstronghold$start.getComponents().isEmpty() || ((StructureStrongholdPieces.Stairs2) mapgenstronghold$start.getComponents().get(
				0)).strongholdPortalRoom == null; mapgenstronghold$start = new MapGenStronghold.Start(this.worldObj, this.rand, chunkX, chunkZ)) {
		}
