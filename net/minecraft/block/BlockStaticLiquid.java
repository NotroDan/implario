...
public class BlockStaticLiquid extends BlockLiquid {
	...
	protected BlockStaticLiquid(Material materialIn) {
		super(materialIn);
		this.setTickRandomly(materialIn == Material.lava);
	}
	...
	public void updateTick(World w, BlockPos pos, IBlockState state, Random rand) {
		...
		if (!w.getGameRules().getBoolean("doFireTick")) return;
		...
		if (i > 0) {
			...
			for (int j = 0; j < i; ++j) {
				...
				if (block.blockMaterial == Material.air) {
					if (this.isSurroundingBlockFlammable(w, blockpos)) {
						w.setBlockState(blockpos, Blocks.fire.getDefaultState());
						return;
					}
				} else if (block.blockMaterial.blocksMovement()) return;
			}
		} else {
			for (int k = 0; k < 3; ++k) {
				BlockPos blockpos1 = pos.add(rand.nextInt(3) - 1, 0, rand.nextInt(3) - 1);

				if (w.isAirBlock(blockpos1.up()) && this.getCanBlockBurn(w, blockpos1))
					w.setBlockState(blockpos1.up(), Blocks.fire.getDefaultState());
			}
		}
	}
	...
	protected boolean isSurroundingBlockFlammable(World worldIn, BlockPos pos) {
		for (EnumFacing enumfacing : EnumFacing.values())
			if (this.getCanBlockBurn(worldIn, pos.offset(enumfacing))) return true;
		return false;
	}
