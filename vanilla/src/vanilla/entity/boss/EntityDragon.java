...
public class EntityDragon extends VanillaEntity implements IBossDisplayData, IComplexEntityRoot, IMob {
	...
	public EntityDragon(World worldIn) {
		...
		this.dragonPartArray = new EntityDragonPart[] {
				this.dragonPartHead = new EntityDragonPart(this, "head", 6.0F, 6.0F), this.dragonPartBody = new EntityDragonPart(this, "body", 8.0F, 8.0F),
				this.dragonPartTail1 = new EntityDragonPart(this, "tail", 4.0F, 4.0F), this.dragonPartTail2 = new EntityDragonPart(this, "tail", 4.0F, 4.0F),
				this.dragonPartTail3 = new EntityDragonPart(this, "tail", 4.0F, 4.0F), this.dragonPartWing1 = new EntityDragonPart(this, "wing", 4.0F, 4.0F),
		...
		if (this.getHealth() <= 0.0F) {
			...
			if (this.isAIDisabled()) {
				...
				if (this.worldObj.isClientSide) {
					...
					Vec3d vec3D = new Vec3d(this.targetX - this.posX, this.targetY - this.posY, this.targetZ - this.posZ).normalize();
					double d15 = -MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F);
					Vec3d vec31D = new Vec3d(MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F), this.motionY, d15).normalize();
					float f5 = ((float) vec31D.dotProduct(vec3D) + 0.5F) / 1.5F;
					...
					float f8 = 0.06F;
					this.move(0.0F, -1.0F, f8 * (f5 * f7 + (1.0F - f7)));
					...
					Vec3d vec32D = new Vec3d(this.motionX, this.motionY, this.motionZ).normalize();
				...
				this.dragonPartHead.setLocationAndAngles(this.posX + (double) (f18 * 5.5F * f2), this.posY + (adouble[1] - adouble1[1]) * 1.0D + (double) (f15 * 5.5F),
				...
				for (int j = 0; j < 3; ++j) {
					...
					entitydragonpart.setLocationAndAngles(this.posX - (double) ((f16 * f23 + f21 * f24) * f2), this.posY + (adouble2[1] - adouble1[1]) * 1.0D - (double) ((f24 + f23) * f15) + 1.5D,
			...
			for (int l1 = j; l1 <= i1; ++l1) {
				for (int i2 = k; i2 <= j1; ++i2) {
					...
					if (block.getMaterial() != Material.air) {
						if (block != Blocks.barrier && block != Blocks.obsidian && block != Blocks.end_stone && block != Blocks.bedrock && block != Blocks.command_block && this.worldObj.getGameRules().getBoolean(
								"mobGriefing")) {
