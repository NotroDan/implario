...
public class EntityEnderman extends EntityMob {
	...
	public EntityEnderman(World worldIn) {
		...
		this.targetTasks.addTask(3, new EntityAINearestAttackableTarget<>(this, EntityEndermite.class, 10,
				true, false, EntityEndermite::isSpawnedByPlayer));
	}
	...
	public void onLivingUpdate() {
		if (this.worldObj.isClientSide) {
			for (int i = 0; i < 2; ++i) {
				this.worldObj.spawnParticle(ParticleType.PORTAL, this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width, this.posY + this.rand.nextDouble() * (double) this.height - 0.25D,
	...
	protected boolean teleportTo(double x, double y, double z) {
		...
		for (int j = 0; j < i; ++j) {
			...
			this.worldObj.spawnParticle(ParticleType.PORTAL, d3, d4, d5, (double) f, (double) f1, (double) f2, new int[0]);
		}
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		...
		if (source.getEntity() == null || !(source.getEntity() instanceof EntityEndermite)) {
			...
			if (source instanceof EntityDamageSource && source.getEntity() instanceof Player) {
				if (source.getEntity() instanceof MPlayer && ((MPlayer) source.getEntity()).theItemInWorldManager.isCreative()) {
					this.setScreaming(false);
				} else {
	...
	static class AIFindPlayer extends EntityAINearestAttackableTarget {
		...
		public boolean shouldExecute() {
			...
			List<Player> list = this.taskOwner.worldObj.getEntitiesWithinAABB(Player.class, this.taskOwner.getEntityBoundingBox().expand(d0, 4.0D, d0), this.targetEntitySelector);
	...
	static class AIPlaceBlock extends EntityAIBase {
		...
		public boolean shouldExecute() {
			return !this.enderman.worldObj.getGameRules().getBoolean(
					"mobGriefing") ? false : this.enderman.getHeldBlockState().getBlock().getMaterial() == Material.air ? false : this.enderman.getRNG().nextInt(2000) == 0;
		}
		...
		private boolean func_179474_a(World worldIn, BlockPos p_179474_2_, Block p_179474_3_, Block p_179474_4_, Block p_179474_5_) {
			return !p_179474_3_.canPlaceBlockAt(worldIn,
					p_179474_2_) ? false : p_179474_4_.getMaterial() != Material.air ? false : p_179474_5_.getMaterial() == Material.air ? false : p_179474_5_.isFullCube();
		}
	...
	static class AITakeBlock extends EntityAIBase {
		...
		public boolean shouldExecute() {
			return !this.enderman.worldObj.getGameRules().getBoolean(
					"mobGriefing") ? false : this.enderman.getHeldBlockState().getBlock().getMaterial() != Material.air ? false : this.enderman.getRNG().nextInt(20) == 0;
		}
