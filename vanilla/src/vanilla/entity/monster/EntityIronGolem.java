...
import vanilla.entity.ai.tasks.village.EntityAIDefendVillage;
import vanilla.entity.ai.tasks.EntityAIHurtByTarget;
import vanilla.entity.ai.tasks.village.EntityAILookAtVillager;
...
import vanilla.entity.ai.tasks.village.EntityAIMoveTowardsTarget;
...
public class EntityIronGolem extends EntityGolem
{
    ...
    protected void updateAITasks()
    {
        if (--this.homeCheckTimer <= 0)
        {
            ...
            this.villageObj = VillageCollection.get(worldObj).getNearestVillage(new BlockPos(this), 32);
        ...
        if (this.motionX * this.motionX + this.motionZ * this.motionZ > 2.500000277905201E-7D && this.rand.nextInt(5) == 0)
        {
            ...
            if (block.getMaterial() != Material.air)
            {
                this.worldObj.spawnParticle(
						ParticleType.BLOCK_CRACK, this.posX + ((double)this.rand.nextFloat() - 0.5D) * (double)this.width, this.getEntityBoundingBox().minY + 0.1D, this.posZ + ((double)this.rand.nextFloat() - 0.5D) * (double)this.width, 4.0D * ((double)this.rand.nextFloat() - 0.5D), 0.5D, ((double)this.rand.nextFloat() - 0.5D) * 4.0D, new int[] {Block.getStateId(iblockstate)});
            }
        }
    }
