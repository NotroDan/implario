...
import vanilla.entity.ai.tasks.EntityAIFindEntityNearest;
import vanilla.entity.ai.tasks.EntityAIFindEntityNearestPlayer;
import vanilla.world.biome.BiomeGenBase;
import vanilla.world.gen.WorldTypes;

public class EntitySlime extends VanillaEntity implements IMob {
	...
	public void onUpdate() {
		if (!this.worldObj.isClientSide && this.worldObj.getDifficulty() == EnumDifficulty.PEACEFUL && this.getSlimeSize() > 0) {
			this.isDead = true;
		}
	...
	public void setDead() {
		...
		if (!this.worldObj.isClientSide && i > 1 && this.getHealth() <= 0.0F) {
	...
	protected void func_175451_e(EntityLivingBase p_175451_1_) {
		...
		if (this.canEntityBeSeen(p_175451_1_) && this.getDistanceSqToEntity(p_175451_1_) < 0.6D * (double) i * 0.6D * (double) i && p_175451_1_.attackEntityFrom(DamageSource.causeMobDamage(this),
	...
	public boolean getCanSpawnHere() {
		...
		if (this.worldObj.getWorldInfo().getTerrainType() == WorldTypes.FLAT && this.rand.nextInt(4) != 1) {
			return false;
		}
		if (this.worldObj.getDifficulty() != EnumDifficulty.PEACEFUL) {
			Biome biomegenbase = this.worldObj.getBiomeGenForCoords(blockpos);

			if (biomegenbase == BiomeGenBase.swampland && this.posY > 50.0D && this.posY < 70.0D && this.rand.nextFloat() < 0.5F && this.rand.nextFloat() < this.worldObj.getCurrentMoonPhaseFactor() && this.worldObj.getLightFromNeighbors(
					new BlockPos(this)) <= this.rand.nextInt(8)) {
