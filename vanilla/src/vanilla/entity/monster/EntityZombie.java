...
public class EntityZombie extends EntityMob {
	...
	public void onLivingUpdate() {
		...
		if (this.isRiding() && this.getAttackTarget() != null && this.ridingEntity instanceof EntityChicken) {
			((VanillaEntity) this.ridingEntity).getNavigator().setPath(this.getNavigator().getPath(), 1.5D);
		}
			...
			if (entitylivingbase != null && this.worldObj.getDifficulty() == EnumDifficulty.HARD && (double) this.rand.nextFloat() < this.getEntityAttribute(reinforcementChance).getAttributeValue()) {
				...
				for (int l = 0; l < 50; ++l) {
					...
					if (World.doesBlockHaveSolidTopSurface(this.worldObj, new BlockPos(i1, j1 - 1, k1)) && this.worldObj.getLightFromNeighbors(new BlockPos(i1, j1, k1)) < 10) {
						...
						if (!this.worldObj.isAnyPlayerWithinRangeAt((double) i1, (double) j1, (double) k1, 7.0D) && this.worldObj.checkNoEntityCollision(entityzombie.getEntityBoundingBox(),
								entityzombie) && this.worldObj.getCollidingBoundingBoxes(entityzombie, entityzombie.getEntityBoundingBox()).isEmpty() && !this.worldObj.isAnyLiquid(
	...
	public void onKillEntity(EntityLivingBase entityLivingIn) {
		...
		if ((this.worldObj.getDifficulty() == EnumDifficulty.NORMAL || this.worldObj.getDifficulty() == EnumDifficulty.HARD) && entityLivingIn instanceof EntityVillager) {
			...
			VanillaEntity entityliving = (VanillaEntity) entityLivingIn;
			EntityZombie entityzombie = new EntityZombie(this.worldObj);
			...
			this.worldObj.spawnEntityInWorld(entityzombie);
			this.worldObj.playAuxSFXAtEntity((Player) null, 1016, new BlockPos((int) this.posX, (int) this.posY, (int) this.posZ), 0);
		}
	}
	...
	protected boolean canPickItemUp(ItemStack stack) {
		return stack.getItem() == Items.egg && this.isChild() && this.isRiding() ? false : super.canPickItemUp(stack);
	}
	...
	protected void convertToVillager() {
		...
		this.worldObj.playAuxSFXAtEntity((Player) null, 1017, new BlockPos((int) this.posX, (int) this.posY, (int) this.posZ), 0);
	}
			...
			for (int k = (int) this.posX - 4; k < (int) this.posX + 4 && j < 14; ++k) {
				for (int l = (int) this.posY - 4; l < (int) this.posY + 4 && j < 14; ++l) {
					for (int i1 = (int) this.posZ - 4; i1 < (int) this.posZ + 4 && j < 14; ++i1) {
						Block block = this.worldObj.getBlockState(blockpos$mutableblockpos.setXyz(k, l, i1)).getBlock();
