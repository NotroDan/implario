...
public class EntityVillager extends EntityAgeable implements IMerchant, INpc {
	...
	private static final EntityVillager.ITradeList[][][][] DEFAULT_TRADE_LIST_MAP = new EntityVillager.ITradeList[][][][] {
			...
			}, {
			{
					...
					new EntityVillager.ListItemForEmeralds(
							VanillaItems.name_tag, new EntityVillager.PriceInfo(20, 22))
			}
			}
	}, {
			{
			...
			new EntityVillager.ListItemForEmeralds(
					VanillaItems.saddle, new EntityVillager.PriceInfo(8, 10))
	}
	}
	}
	};
	...
	protected void updateAITasks() {
		if (--this.randomTickDivider <= 0) {
			...
			VillageCollection.get(this.worldObj).addToVillagerPositionList(blockpos);
			this.randomTickDivider = 70 + this.rand.nextInt(50);
			this.villageObj = VillageCollection.get(this.worldObj).getNearestVillage(blockpos, 32);
	...
	public boolean interact(Player player) {
		...
		if (!flag && this.isEntityAlive() && !this.isTrading() && !this.isChild()) {
			if (!this.worldObj.isClientSide && (this.buyingList == null || this.buyingList.size() > 0)) {
				...
				player.openGui(IMerchant.class, this);
			}
	...
	private void spawnParticles(ParticleType particleType) {
		for (int i = 0; i < 5; ++i) {
			...
			this.worldObj.spawnParticle(particleType, this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width,
	...
	static class ItemAndEmeraldToItem implements EntityVillager.ITradeList {
		...
		public void modifyMerchantRecipeList(MerchantRecipeList recipeList, Random random) {
			...
			recipeList.add(new MerchantRecipe(new ItemStack(this.field_179411_a.getItem(), i, this.field_179411_a.getMetadata()), new ItemStack(Items.emerald),
	...
	static class PriceInfo extends Tuple<Integer, Integer> {
		...
		public int getPrice(Random rand) {
			return ((Integer) this.getFirst()).intValue() >= ((Integer) this.getSecond()).intValue() ? ((Integer) this.getFirst()).intValue() : ((Integer) this.getFirst()).intValue() + rand.nextInt(
