...
public abstract class MobSpawnerBaseLogic {
	...
	private boolean isActivated() {
		...
		return this.getSpawnerWorld().isAnyPlayerWithinRangeAt((double) blockpos.getX() + 0.5D, (double) blockpos.getY() + 0.5D, (double) blockpos.getZ() + 0.5D,
	...
	public void updateSpawner() {
		if (this.isActivated()) {
			...
			if (this.getSpawnerWorld().isClientSide) {
				...
				this.getSpawnerWorld().spawnParticle(ParticleType.SMOKE_NORMAL, d3, d4, d5, 0.0D, 0.0D, 0.0D, new int[0]);
				this.getSpawnerWorld().spawnParticle(ParticleType.FLAME, d3, d4, d5, 0.0D, 0.0D, 0.0D, new int[0]);
				...
				for (int i = 0; i < this.spawnCount; ++i) {
					...
					int j = this.getSpawnerWorld().getEntitiesWithinAABB(entity.getClass(),
							new AxisAlignedBB((double) blockpos.getX(), (double) blockpos.getY(), (double) blockpos.getZ(), (double) (blockpos.getX() + 1), (double) (blockpos.getY() + 1),
									(double) (blockpos.getZ() + 1)).expand((double) this.spawnRange, (double) this.spawnRange, (double) this.spawnRange)).size();
					...
					VanillaEntity entityliving = entity instanceof VanillaEntity ? (VanillaEntity) entity : null;
	...
	private Entity spawnNewEntity(Entity entityIn, boolean spawn) {
		if (this.getRandomEntity() != null) {
			...
			if (entityIn instanceof VanillaEntity) {
				((VanillaEntity) entityIn).onInitialSpawn(entityIn.worldObj.getDifficultyForLocation(new BlockPos(entityIn)), (IEntityLivingData) null);
			}
	...
	public boolean setDelayToMin(int delay) {
		if (delay == 1 && this.getSpawnerWorld().isClientSide) {
			this.spawnDelay = this.minSpawnDelay;
			return true;
		}
		return false;
