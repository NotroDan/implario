...
public class FlatGeneratorInfo {
	...
	private static FlatLayerInfo createLayerInfo(int p_180715_0_, String p_180715_1_, int p_180715_2_) {
		...
		if (astring.length == 2) {
			try {
				...
				if (p_180715_2_ + i >= 256) i = 256 - p_180715_2_;
				if (i < 0) i = 0;
	...
	private static List<FlatLayerInfo> createLayerInfos(int p_180716_0_, String string) {
		...
		for (String s : astring) {
			FlatLayerInfo flatlayerinfo = createLayerInfo(p_180716_0_, s, i);

			if (flatlayerinfo == null) return null;
	...
	public static FlatGeneratorInfo createFlatGeneratorFromString(String string) {
		if (string == null) return getDefaultFlatGenerator();
		String[] astring = string.split(";", -1);
		int i = astring.length == 1 ? 0 : MathHelper.parseIntWithDefault(astring[0], 0);

		if (i < 0 || i > 3) return getDefaultFlatGenerator();
		FlatGeneratorInfo flatgeneratorinfo = new FlatGeneratorInfo();
		int j = astring.length == 1 ? 0 : 1;
		List<FlatLayerInfo> list = createLayerInfos(i, astring[j++]);

		if (list == null || list.isEmpty()) return getDefaultFlatGenerator();
		flatgeneratorinfo.getFlatLayers().addAll(list);
		flatgeneratorinfo.func_82645_d();
		int k = BiomeGenBase.plains.getLegacyId();

		if (i > 0 && astring.length > j) k = MathHelper.parseIntWithDefault(astring[j++], k);
	...
	public static FlatGeneratorInfo getDefaultFlatGenerator() {
		FlatGeneratorInfo flatgeneratorinfo = new FlatGeneratorInfo();
		flatgeneratorinfo.setBiome(BiomeGenBase.plains.getLegacyId());
