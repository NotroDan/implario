...
public class ChunkProviderGenerate implements VanillaChunkProvider {
	...
	public void replaceBlocksForBiome(int p_180517_1_, int p_180517_2_, ChunkPrimer p_180517_3_, Biome[] p_180517_4_) {
		...
		for (int i = 0; i < 16; ++i) {
			for (int j = 0; j < 16; ++j) {
				BiomeGenBase biomegenbase = BiomeGenBase.toGenBase(p_180517_4_[j + i * 16]);
	...
	public Chunk provideChunk(int x, int z) {
		...
		for (int i = 0; i < abyte.length; ++i) {
			abyte[i] = (byte) this.biomesForGeneration[i].getLegacyId();
		}
	...
	private void func_147423_a(int p_147423_1_, int p_147423_2_, int p_147423_3_) {
		this.field_147426_g = this.noiseGen6.generateNoiseOctaves(this.field_147426_g, p_147423_1_, p_147423_3_, 5, 5, (double) this.settings.depthNoiseScaleX, (double) this.settings.depthNoiseScaleZ,
				(double) this.settings.depthNoiseScaleExponent);
		float f = this.settings.coordinateScale;
		float f1 = this.settings.heightScale;
		this.field_147427_d = this.field_147429_l.generateNoiseOctaves(this.field_147427_d, p_147423_1_, p_147423_2_, p_147423_3_, 5, 33, 5, (double) (f / this.settings.mainNoiseScaleX),
		...
		for (int k = 0; k < 5; ++k) {
			for (int l = 0; l < 5; ++l) {
				...
				float f4 = 0.0F;
				int i1 = 2;
				BiomeGenBase biomegenbase = BiomeGenBase.toGenBase(this.biomesForGeneration[k + 2 + (l + 2) * 10]);

				for (int j1 = -i1; j1 <= i1; ++j1) {
					for (int k1 = -i1; k1 <= i1; ++k1) {
						BiomeGenBase biomegenbase1 = BiomeGenBase.toGenBase(this.biomesForGeneration[k + j1 + 2 + (l + k1 + 2) * 10]);
						...
						if (this.field_177475_o == WorldTypes.AMPLIFIED && f5 > 0.0F) {
							f5 = 1.0F + f5 * 2.0F;
							f6 = 1.0F + f6 * 4.0F;
						}
	...
	public void populate(IChunkProvider p_73153_1_, int p_73153_2_, int p_73153_3_) {
		...
		BiomeGenBase biomegenbase = BiomeGenBase.toGenBase(this.worldObj.getBiomeGenForCoords(blockpos.add(16, 0, 16)));
	...
	public List<SpawnListEntry> getPossibleCreatures(EnumCreatureType creatureType, BlockPos pos) {
		BiomeGenBase biomegenbase = BiomeGenBase.toGenBase(this.worldObj.getBiomeGenForCoords(pos));
